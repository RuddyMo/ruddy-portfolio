...
onMounted(() => {
  ...
  if (Array.isArray(props.backgroundColor) && props.backgroundColor.length === 4) {
    gl.clearColor(
      props.backgroundColor[0] ?? 0,
      props.backgroundColor[1] ?? 0,
      props.backgroundColor[2] ?? 0,
      props.backgroundColor[3] ?? 0,
    );
  } else {
  ...
  function updateMouse(e: MouseEvent | TouchEvent) {
    ...
    // Utiliser les coordonnées globales de la fenêtre
  ...
  // Écouter les événements sur le document pour capturer tous les mouvements
    ...
    lines.forEach((line) => {
      ...
      for (let i = 1; i < line.points.length; i++) {
        ...
        if (isFinite(props.maxAge) && props.maxAge > 0) {
          ...
          currentPoint.lerp(previousPoint, alpha);
        } else {
          currentPoint.lerp(previousPoint, 0.9);
        }
      }
